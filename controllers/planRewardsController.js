...
const PlanReward = require("../models/planReward.js");
...
exports.getAllRewards = handleErrorAsync(async (req, res, next) => {
    ...
    const totalCount = await PlanReward.countDocuments(query);

    //  const totalPages = Math.ceil(totalCount / itemsPerPage);

    let resPlanReward = await PlanReward.find(query)
        .sort(tSort);
    ...
    Success(res, "請求成功，回傳所需數據", { data: resPlanReward });
    /*
      #swagger.tags =  ['計畫管理']
...
exports.getRewards = handleErrorAsync(async (req, res, next) => {
    const { timeSort, keyWord, page = 1, limit = 10 } = req.query;
    const { plan_id } = req.params;
    const tSort = "-publicAt";
    let query = {};
    ...
    Success(res, "請求成功，回傳所需數據", { data: resPlanReward, pagination });
    /*
      #swagger.tags =  ['計畫管理']
      #swagger.path = '/v1/api/news'
      #swagger.method = 'get'
...
exports.getRewardByID = handleErrorAsync(async (req, res, next) => {
    ...
    let resPlanRewards = await PlanReward.findById(id);

    // 設定分頁信息

    Success(res, "請求成功，回傳所需數據", { data: resPlanRewards });

});
exports.createReward = handleErrorAsync(async (req, res, next) => {
    const { plan_id } = req.params;
    const { updateData } = req;
    if (!plan_id || !updateData) {
        return next(appError("缺少必要的參數或資料", next, 400, 1002));
    }
    if (!plan_id.trim()) {
        return next(appError("id欄位不能為空值！", next, 400, 1002));
    }
    ...
    Object.keys(updateData).forEach((key) => {
        ...
        if ((key === "unit") && !updateData[key].trim()) {
            return next(appError(`unit 欄位不能為空值！`, next, 400, 1002));
        }
        if ((key === "content") && !updateData[key].trim()) {
            return next(appError(`content 欄位不能為空值！`, next, 400, 1002));
        }
    ...
    if (req.files && req.files.length > 0) {
        ...
        blobStream.on("finish", async () => {
            try {
                ...
                // 儲存到 MongoDB
                const resPlanReward = await PlanReward.create({ ...filteredData, ...updateData });

                if (!resPlanReward) {
                    throw appError("建立失敗!", next, 400);
                }

                Success(res, "已建立貼文", resPlanReward, 201);
            } catch (error) {
                next(error);
            }
        });
...
exports.updateRewardByID = handleErrorAsync(async (req, res, next) => {
    const { plan_id, id } = req.params;
    const { updateData } = req;
    if (!plan_id || !updateData) {
        return next(appError("缺少必要的參數或資料", next, 400, 1002));
    ...
    if (req.files && req.files.length > 0) {
        ...
        blobStream.on("finish", async () => {
            try {
                ...
                // 儲存到 MongoDB
                const resPlanReward = await PlanReward.findByIdAndUpdate(
                    id,
                ...
                Success(res, "已更新計畫回饋", resPlanReward, 201);
            } catch (error) {
                next(error);
            }
        });
...
exports.deleteRewardByID = handleErrorAsync(async (req, res, next) => {
    ...
    const resPlanReward = await PlanReward.findByIdAndDelete(
        id,
    ...
    Success(res, `最新消息:${resPlanReward.title}資料已刪除`);
});
